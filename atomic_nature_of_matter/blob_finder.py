...
class BlobFinder:
    ...
    def __init__(self, pic, tau):
        ...
        # dimensions as pic.
        marked = [[False for y in range(self.pic.height())] for x in range(self.pic.width())]
    ...
    def _findBlob(self, pic, tau, i, j, marked, blob):
        ...
        if (i < 0 or j < 0 or i >= pic.width() or j >= pic.height() or marked[i][j]):
            return
...
def _main():
    ...
    picture = Picture(sys.argv[3])
    finder = BlobFinder(picture, tau)
    beads = finder.getBeads(P)
    blobs = finder.getBeads(1)
